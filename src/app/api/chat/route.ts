...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const ownerSecret = env.OWNER_SECRET_KEY;
		...
		const isAuthCommand =
			lowerMessage.startsWith('/sudo auth ') ||
			lowerMessage.startsWith('/auth ');
		const isLogoutCommand =
			lowerMessage === '/sudo logout' || lowerMessage === '/logout';
							...
							controller.enqueue(
								encoder.encode(
									`data: ${JSON.stringify({
										...
										data: {
											content:
												'🔓 **[System Security] Owner authentication successful.** System administrative clearance granted. Admin cookie initialized.',
										},
									})}\n\n`,
								),
							);
							...
							controller.enqueue(
								encoder.encode(
									`data: ${JSON.stringify({
										...
										data: {
											content:
												'🔒 **[System Security] Access denied.** Invalid administrative key protocol.',
										},
									})}\n\n`,
								),
							);
						...
						controller.enqueue(
							encoder.encode(
								`data: ${JSON.stringify({
									...
									data: {
										content:
											'🔒 **[System Security] Administrative session terminated.** Admin credentials cleared successfully.',
									},
								})}\n\n`,
							),
						);
		...
		const openRouter = new OpenRouter({
			...
			httpReferer: env.NEXT_PUBLIC_SITE_URL || DOMAINS.canonical,
			...
			async start(controller) {
				...
				const safeEnqueue = (data: Uint8Array) => {
					try {
						...
						log.warn('[Agent Router] Stream closed during enqueue:', { err: e });
					}
				};
				...
				const getMaskedModel = (rawModel: string): string => {
					...
					if (rawModel.includes('gemma') || rawModel.includes('glm'))
						return 'Weaver Marketing v3 (Custom)';
					if (rawModel.includes('qwen') || rawModel.includes('gpt'))
						return 'Weaver Systems Coder v3 (Custom)';
				...
				for (const modelCandidate of modelExecutionList) {
					try {
						...
						// We set a 6-second timeout limit to get the first chunk of data. This keeps the experience snappy
						// and ensures rapid failover to the next candidate model in the chain.
							...
							new Promise<never>((_, reject) =>
								setTimeout(
									() =>
										reject(
											new Error(
												`Timeout waiting for first token from ${modelCandidate}`,
											),
										),
						...
						while (!chunkResult.done) {
							...
							if (!success) {
								...
								if (selectedModel !== targetModel) {
									...
									safeEnqueue(
										encoder.encode(
											`data: ${JSON.stringify(fallbackStats)}\n\n`,
										),
									);
								}
							}
		...
		log.error(
			`❌ Agent Chat Route Exception - IP: ${clientIP}, Response Time: ${responseTime}ms`,
			{ err: isProd ? new Error((error as any)?.message || String(error)) : error }
		);
