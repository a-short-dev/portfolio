...
function runInputGuardrails(message: string): GuardrailResult {
  if (message.length < MIN_MESSAGE_LENGTH) {
    return {
      ...
      fallbackResponse: "Hello! Please type a slightly longer query so I can assist you better.",
    };
  }
  ...
  if (message.length > MAX_MESSAGE_LENGTH) {
    return {
      ...
      fallbackResponse: "Your message is a bit too long. Please keep it concise and under 600 characters.",
    };
  }
  ...
  for (const keyword of promptInjectionKeywords) {
    if (lowerMessage.includes(keyword)) {
      return {
        ...
        fallbackResponse: "I am programmed to assist you with exploring Oluwaleke Abiodun's portfolio and technical background. I cannot bypass my systems or instructions.",
      };
    }
  }
  ...
  for (const pattern of BLOCKED_PATTERNS) {
    if (pattern.test(message)) {
      return {
        ...
        fallbackResponse: "I cannot assist with queries containing potentially unsafe, inappropriate, or systems-compromising content. Let me know if you want to see my portfolio projects instead!",
      };
    }
  }

  // Relevance check: ensure the query is somewhat related to technology, hiring, engineering, or this website
  const genericWords = ["hello", "hi", "hey", "who are you", "what can you do", "help", "yes", "no"];
  const isGeneric = genericWords.some((word) => lowerMessage === word || lowerMessage.startsWith(word + " "));

  if (!isGeneric) {
    const technologyKeywords = [
      "portfolio", "work", "project", "hire", "job", "contract", "developer", "engineer", "resume", "cv",
      "code", "rust", "kotlin", "swift", "typescript", "react", "next", "ios", "android", "mobile", "web",
      "e-commerce", "finance", "database", "backend", "frontend", "system", "scale", "performance", "api",
      "laundry", "apartments", "crime", "oyo", "ncc", "veris", "mood", "vibe", "consulting", "pricing", "rate"
    ];
    const hasTechKeyword = technologyKeywords.some((keyword) => lowerMessage.includes(keyword));
    if (!hasTechKeyword && lowerMessage.split(/\s+/).length > 4) {
      return {
        isSafe: false,
        reason: "Off-topic check failed",
        fallbackResponse: "I am Oluwaleke's AI Assistant, built specifically to guide you through his engineering capabilities, active projects, and how he can help you deploy high-performance applications. Let's keep discussions focused on engineering, systems, or professional inquiries!",
      };
    }
  }
...
async function getAgentMemory(): Promise<AgentMemory> {
  try {
    ...
    try {
      ...
      await fs.writeFile(MEMORY_FILE_PATH, JSON.stringify(defaultMemory, null, 2), "utf-8");
    } catch (e) {
...
async function learnFromQuestion(message: string): Promise<void> {
  try {
    ...
    if (lower.includes("shop") || lower.includes("store") || lower.includes("e-commerce") || lower.includes("buy") || lower.includes("sell")) {
      memory.topicsOfInterest["e-commerce"] = (memory.topicsOfInterest["e-commerce"] || 0) + 1;
      detectedTopic = "e-commerce";
    }
    if (lower.includes("mobile") || lower.includes("ios") || lower.includes("android") || lower.includes("app") || lower.includes("react native")) {
      memory.topicsOfInterest["mobile-apps"] = (memory.topicsOfInterest["mobile-apps"] || 0) + 1;
      detectedTopic = "mobile-apps";
    }
    if (lower.includes("loan") || lower.includes("finance") || lower.includes("fintech") || lower.includes("bank") || lower.includes("credit")) {
      memory.topicsOfInterest["fintech"] = (memory.topicsOfInterest["fintech"] || 0) + 1;
      detectedTopic = "fintech";
    }
    if (lower.includes("rust") || lower.includes("performance") || lower.includes("low-level") || lower.includes("systems") || lower.includes("speed")) {
      memory.topicsOfInterest["rust-systems"] = (memory.topicsOfInterest["rust-systems"] || 0) + 1;
      detectedTopic = "rust-systems";
    }
    if (lower.includes("hire") || lower.includes("consult") || lower.includes("book") || lower.includes("work with")) {
      memory.topicsOfInterest["consultation"] = (memory.topicsOfInterest["consultation"] || 0) + 1;
      detectedTopic = "consultation";
    }
    if (lower.includes("cost") || lower.includes("price") || lower.includes("rate") || lower.includes("quote")) {
      memory.topicsOfInterest["pricing-inquiries"] = (memory.topicsOfInterest["pricing-inquiries"] || 0) + 1;
...
function routeToModel(message: string): RouteDecision {
  ...
  const marketingKeywords = [
    "hire", "consult", "cost", "pricing", "rate", "quote", "build me", "make a",
    "why should I", "sell me", "portfolio", "projects", "ecommerce", "fashion",
    "laundry", "property", "booking", "flexiti", "uvo", "veris", "moodjournal"
  ];
  ...
  if (isMarketing) {
    return {
      ...
      category: "marketing",
      explanation: "Routed to Gemma-4-31B for elite copywriting and high-impact marketing response.",
    };
  }

  // 2. Deep Technical / Systems / Code questions (Rust, low level memory, concurrency, APIs)
  const technicalKeywords = [
    "rust", "performance", "memory", "efficient", "concurrency", "architecture",
    "database", "postgresql", "typescript", "microservices", "scale", "system",
    "low level", "refactor", "complexity", "algorithm"
  ];
  ...
  if (isTechnical) {
    return {
      ...
      category: "technical",
      explanation: "Routed to GPT-OSS-120B for high-fidelity technical depth and systems explanation.",
    };
  }
  ...
  return {
    ...
    category: "general",
    explanation: "Routed to Gemma-4-31B for elite general-purpose conversation.",
  };
}

// Fallback chain in case the specific target model is currently rate-limited or offline
const MODEL_FALLBACKS: Record<string, string[]> = {
  "google/gemma-4-31b-it:free": ["nvidia/nemotron-3-super-120b-a12b:free", "openai/gpt-oss-120b:free", "google/gemma-4-26b-a4b-it:free"],
  "openai/gpt-oss-120b:free": ["google/gemma-4-26b-a4b-it:free", "nvidia/nemotron-3-super-120b-a12b:free", "google/gemma-4-31b-it:free"],
  "nvidia/nemotron-3-super-120b-a12b:free": ["google/gemma-4-31b-it:free", "openai/gpt-oss-120b:free", "google/gemma-4-26b-a4b-it:free"],
};

// ── Agent Prompt Customization ────────────────────────────────────────────────

function constructSystemPrompt(visitorType: string, memory: AgentMemory, category: string): string {
  const baseContext = WEBSITE_CONTEXT;
  const learnedNotes = memory.learnedInsights.map((insight) => `- ${insight}`).join("\n");
...
Provide direct, highly technical architecture feedback, system status data, and performance stats. Skip marketing pitches entirely.`;
    ...
    persona = `
You are Oluwaleke Abiodun's expert Sales and Marketing AI Agent.
Your core objective is to sell Oluwaleke's world-class engineering capabilities to prospective clients!
*   Highlight his specialization in **performance, memory efficiency, and bulletproof security**.
*   Directly reference key contract systems like **Novoct Planet (Fashion E-commerce)**, **Flexiti (Fintech Consumer Finance)**, and **Canwee Apartments (Luxury Booking Platforms)** as proof of commercial pedigree.
*   Proactively nudge them toward a WhatsApp consultation at the end of your message: **https://wa.me/2349165913234** (WhatsApp: +234 916 591 3234).
*   Keep your response extremely concise, premium, and compelling (maximum 3-4 sentences).`;
  } else {
    persona = `
You are Oluwaleke Abiodun's professional AI Assistant.
Explore the visitor's goals, answer technical or background questions, and guide them politely toward hire or professional consulting.
*   End your response with an actionable next step or a thought-provoking question.
*   Keep responses concise, elegant, and friendly (maximum 3 sentences).`;
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!guardrail.isSafe) {
      console.warn(`[Guardrails Triggered] IP: ${clientIP}, Reason: ${guardrail.reason}`);
      return NextResponse.json(
    ...
    const visitorType = message.toLowerCase().includes("oluwaleke") || message.toLowerCase().includes("a_short_dev") ? "owner" : "visitor";

    // Construct final prompt
    const systemPrompt = constructSystemPrompt(visitorType, memory, routing.category);
    ...
    if (!streamingResponseStream) {
      ...
      streamingResponseStream = await openRouter.chat.send({
        chatRequest: {
          model: selectedModel,
          messages: [
            { role: "system", content: systemPrompt },
        ...
        httpReferer: process.env.NEXT_PUBLIC_SITE_URL || "https://oluwaleke.dev",
        appTitle: "Oluwaleke Portfolio AI Assistant",
      });
    }
    ...
    const responseStream = new ReadableStream({
      async start(controller) {
        ...
        try {
          for await (const chunk of streamingResponseStream) {
    ...
    return NextResponse.json(
      {
        error: "I ran into an issue setting up my routing system. Let's try again in a second!",
      },
      { status: 500 },
    );
  }
