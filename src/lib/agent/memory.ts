...
export async function getAgentMemory(): Promise<AgentMemory> {
	try {
		...
		return {
			totalQuestionsCount:
				data.totalQuestionsCount ?? defaultMemory.totalQuestionsCount,
			topicsOfInterest: {
		...
		log.error('Failed to get agent memory from Redis:', {
			err: isProd
...
export async function learnFromQuestion(message: string): Promise<void> {
	...
	try {
		...
		const isProd = env.NODE_ENV === 'production';
		log.error('Error writing agent memory to Redis:', {
			err: isProd
				? new Error(error instanceof Error ? error.message : String(error))
				: error,
		});
