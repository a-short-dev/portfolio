...
async function runBenchmark() {
	...
	log.info(`${BOLD}OFFLINE METRICS (over ${iterations} iterations):${RESET}`);
	log.info(
	...
	if (!OPENROUTER_API_KEY) {
		...
		log.info(`  ${BOLD}Prompt:${RESET} "${testPrompt}"`);
		...
		try {
			...
			log.info(`  ${RED}✘ Live API Benchmark Failed:${RESET} ${err.message}`);
			log.info(
